using System;
using System.Collections.Generic;
using System.Linq;
using CaChepCosmeticMVC2.Data;
using CaChepCosmeticMVC2.Models.DataModels;
using CaChepCosmeticMVC2.Models.IDataModels;
using Microsoft.EntityFrameworkCore;

namespace CaChepCosmeticMVC2.Service
{
    public class ChiTieuSv : IChiTieu
    {
        private readonly ApplicationDbContext _context;

        public ChiTieuSv(ApplicationDbContext context)
        {
            _context = context;
        }

        public IEnumerable<ChiTieu> GetAll()
        {
            throw new NotImplementedException();
        }

        public ChiTieu GetOneById(int? id)
        {
            throw new NotImplementedException();
        }

        public void New(ChiTieu a)
        {
            throw new NotImplementedException();
        }

        public void Edit(ChiTieu a)
        {
            throw new NotImplementedException();
        }

        public void Delete(int id)
        {
            throw new NotImplementedException();
        }
    }
}